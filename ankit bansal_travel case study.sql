...
-----write a query to get below output

select u.segment,COUNT(DISTINCT(u.user_id)) AS no_of_user,
COUNT(DISTINCT case when b.line_of_business = 'Flight' AND b.booking_date BETWEEN '2022-04-01' AND '2022-04-30' then b.user_id END) AS colm2
...
---write a query to identify users whoes first booking was hotel booking

SELECT * FROM (SELECT *, RANK() Over(partition by user_id order by booking_date) as rn
FROM booking_table)a
WHERE a.rn=1 AND a. line_of_business='Hotel';

---write a query to calculate the days between first & last booking date of each user

SELECT user_id,
MIN(booking_date) as first_date,
MAX(booking_date) as last_date,
DATEDIFF(day,MIN(booking_date),MAX(booking_date)) as gap
FROM booking_table
GROUP BY user_id;


---write a query to count the no of flight & hotel bookings in each of the user segments for the year 2022
